const { chromium } = require('playwright');
const fs = require('fs');

const PROJECT_ID = 'CF6DA5CB5BE8C843FE37526843D3E126';
const URL = `https://www.pressplay.cc/member/learning/projects/${PROJECT_ID}/articles`;
const COOKIE = process.env.PP_COOKIE || `JAccessToken=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpYXQiOjE3NzcwMjM1OTAsImV4cCI6MTc3OTYxNTU5MCwiZGF0YSI6eyJhY2Nlc3NUb2tlbiI6IjZkZjgzMzljYjE3MjY1Mjg2MjA0MmRhZTk1ZGU2M2Q0NzZiYzM5YTAiLCJtZW1iZXJfaWQiOiI5NkMwRjY4RUIzNjg1NTA4NUM2RjdEQTczMDM3OUQ2MCJ9fQ.UZuRkUcGGUOTVAZlBv92ufssawOe3DIGKuMGErb5-wE; PHPSESSID=568jb4qoda7bj7faqb51ps019r; pp_identify=72dff85f0f6c397867acca3c2995457d; salt=19dbedb80df223f-07f45531a5719c8-26061e51-1a298c-19dbedb80e0284b`;

async function main() {
  const browser = await chromium.launch({ headless: true });
  const context = await browser.newContext();
  const cookiesArr = [];
  COOKIE.split(';').forEach(c => {
    const parts = c.trim().split('=');
    if (parts.length >= 2) cookiesArr.push({ name: parts[0], value: parts.slice(1).join('='), domain: '.pressplay.cc', path: '/' });
  });
  await context.addCookies(cookiesArr);

  const page = await context.newPage();
  await page.goto(URL, { waitUntil: 'networkidle' });

  let articles = [];

  for (let p = 1; p <= 100; p++) {
      console.log(`Processing Page ${p}...`);
      
      // Extract current articles
      const pageArticles = await page.evaluate(() => {
          const links = Array.from(document.querySelectorAll('a[href*="/articles/"]'));
          return links.map(item => {
              const id = item.href.split('/').pop().split('?')[0];
              const title = item.innerText.trim().split('\n')[0];
              const card = item.closest('.article-card');
              // Try multiple ways to get date
              const dateEl = card ? (card.querySelector('[itemprop="datePublished"]') || card.querySelector('[data-type="time"]') || card.querySelector('.article-card-metadata span')) : null;
              const date = dateEl ? (dateEl.content || dateEl.innerText) : '';
              return { id, title, date };
          }).filter(a => a.id.length > 20);
      });

      pageArticles.forEach(a => {
          if (!articles.find(exist => exist.id === a.id)) articles.push(a);
      });
      console.log(`Found ${pageArticles.length} articles. Total unique: ${articles.length}. Earliest on page: ${pageArticles[pageArticles.length-1]?.date}`);

      // Stop if we hit 2024 (e.g. "1年前" or "2024/...")
      const lastDate = pageArticles[pageArticles.length-1]?.date || '';
      if (lastDate.includes('2年前') || lastDate.includes('2023')) {
          console.log("Reached historical target. Stopping.");
          break;
      }

      // Click Next
      const nextBtn = await page.$('.pp-pagination-item[rel="next"]');
      if (nextBtn) {
          console.log("Clicking Next...");
          await nextBtn.click();
          await page.waitForTimeout(3000); 
      } else {
          console.log("No Next button found. Stopping.");
          break;
      }
  }

  fs.writeFileSync('data/puhui_urls_paginated.json', JSON.stringify(articles, null, 2));
  console.log(`Saved ${articles.length} articles.`);
  await browser.close();
}

main().catch(console.error);
